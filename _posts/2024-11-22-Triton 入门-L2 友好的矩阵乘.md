...
本文基于 Triton 逐步实现 matmul 算子。参考了官方 [tutorials](https://github.com/triton-lang/triton/blob/v2.1.0/python/tutorials/03-matrix-multiplication.py)。[原理](https://triton-lang.org/main/getting-started/tutorials/03-matrix-multiplication.html#l2-cache-optimizations)在这，大意是重新调整 `pid` 到 `pid_m` `pid_n` 的映射，使得对应到 C 矩阵的下标具有局部性，优化 B 矩阵的访存。我在官方示例的基础上增加了 `GROUP_SIZE_M == 0` 时使用 naive 分配策略。
