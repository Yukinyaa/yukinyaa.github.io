...
这个文件是整个库的核心。需要前置介绍的是，这个算法使用了 WarpSpecialization (WS)，这项技术最早出现在 SC'11 [CudaDMA: Optimizing GPU Memory Bandwidth via Warp Specialization](https://ieeexplore.ieee.org/document/6114413) 这篇文章中。另一篇推荐阅读的文章是 HPCA'24 的 [WASP: Exploiting GPU Pipeline Parallelism with Hardware-Accelerated Automatic Warp Specialization](https://ieeexplore.ieee.org/document/10476447)。在 2011 年 WS 其实是很不靠谱的做法，当时 gpu 多线程能力太差。但是 Hopper 中引入了以 TMA 为首的大量异步操作（NV 称之为第一代异步 GPU），WS 得以大放光彩。可能 DeepGEMM 胜过 cutlass 的很大原因是后者因为架构包袱没有及时跟上高效的 WS 算法（未查证 cutlass 实现）？
